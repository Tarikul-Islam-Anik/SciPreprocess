...
class OutputValidator:
    ...
    def _validate_top_level(self, data: Dict[str, Any]):
        """Validate top-level structure."""
        required_keys = ['metadata', 'abstract', 'sections', 'figures', 
                        'tables', 'equations', 'references', 'acronyms']
    ...
    def _validate_sections(self, sections: List[Dict[str, Any]]):
        ...
        for i, section in enumerate(sections):
            ...
            if 'text' not in section:
                ...
                self.warnings.append(f"sections[{i}] ('{section.get('heading', '?')}') has empty text")
    ...
    def _validate_figures(self, figures: List[Dict[str, Any]]):
        ...
        for i, figure in enumerate(figures):
            ...
            if 'type' in figure and figure['type'] != 'figure':
                self.warnings.append(f"figures[{i}].type should be 'figure', got '{figure['type']}'")
    ...
    def _validate_equations(self, equations: List[Dict[str, Any]]):
        ...
        for i, equation in enumerate(equations):
            ...
            if 'type' in equation and equation['type'] != 'equation':
                self.warnings.append(f"equations[{i}].type should be 'equation', got '{equation['type']}'")
    ...
    def _validate_acronyms(self, acronyms: Dict[str, str]):
        ...
        for key, value in acronyms.items():
            ...
            if not isinstance(value, str):
                self.errors.append(f"acronyms['{key}'] must be a string, got {type(value).__name__}")
