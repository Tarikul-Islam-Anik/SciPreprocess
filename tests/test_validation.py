...
class TestOutputValidator:
    ...
    def test_valid_output(self):
        ...
        valid_data = {
            ...
            'figures': [
                {'type': 'figure', 'number': '1', 'caption': 'Test figure', 'page': 1}
            ],
            'tables': [
                {'type': 'table', 'number': '1', 'caption': 'Test table', 'page': 2}
            ],
            'equations': [
                {'type': 'equation', 'number': '1', 'page': 3}
            ],
            'references': [
                {'number': '1', 'text': 'Author et al. (2023). Paper title.'}
            ],
            'acronyms': {
    ...
    def test_duplicate_warnings(self):
        ...
        validator = OutputValidator(strict=False)
        result = validator.validate(data_with_duplicates)
