...
from .features import tfidf_features, maybe_build_embeddings
from .models import ParsedDocument
from .parsers import ingest
from .preprocessing import clean_text, ocr_image_to_text
from .sectioning import split_into_sections, split_into_sections_with_toc, semantic_chunk_sections
from .utils import load_spacy_model, ensure_nltk_resources, print_availability_status
...
class PreprocessingPipeline:
    ...
    def _assemble_document_json(
        ...
        # Extract abstract
        abstract = next(
            (s['text'] for s in sections if s['heading'].lower() == 'abstract'),
            ''
        )
    ...
    def preprocess_documents(self, file_paths: List[str], lower: bool = False) -> Dict[str, Any]:
        ...
        X, vectorizer = tfidf_features(corpus)
        ...
        for j in docs_json:
            ...
            chunks = semantic_chunk_sections(
                secs,
                *self.config.chunk_target_sentences
            )
        ...
        if self.config.use_semantic_embeddings:
            ...
            embeddings, index = maybe_build_embeddings(
                flat_chunks,
        ...
        return {
            'documents': docs_json,
            'tfidf': {'X': X, 'vectorizer': vectorizer},
            'chunks': all_chunks,
