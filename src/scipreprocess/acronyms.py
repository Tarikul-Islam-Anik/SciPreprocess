...
def detect_acronyms(text: str, nlp_model: Optional[any] = None) -> Dict[str, str]:
    ...
    if nlp_model is not None and hasattr(nlp_model, 'pipe_names'):
        if 'abbreviation_detector' in nlp_model.pipe_names or \
           any('AbbreviationDetector' in p for p in nlp_model.pipe_names):
            try:
                doc = nlp_model(text)
...
def expand_acronyms(text: str, mapping: Dict[str, str]) -> str:
    ...
    pattern = re.compile(
        r'\b(' + '|'.join(re.escape(k) for k in sorted(mapping.keys(), key=len, reverse=True)) + r')\b'
    )
